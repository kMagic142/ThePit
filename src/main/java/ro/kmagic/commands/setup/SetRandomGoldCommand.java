...
public class SetRandomGoldCommand extends SubCommand implements Listener {
    private final ItemStack wand;
    
    public SetRandomGoldCommand() {
        super("setRandomGold", SetupUtils.getSetupPermissions());
        this.wand = Materials.GOLD_INGOT.getItem().setUnbreakable(true).enchantment().addUnsafe(Enchantment.DURABILITY, 1).flag().add(ItemFlag.values()).setDisplayName("&f&l[&a&lThePit&f&l] &ePit Random Gold Wand").setLore(Arrays.asList("&7&oSetup item", " ", "&8Right-Click&7: &fAdd block")).build();
    ...
    public void execute(final CommandSender commandSender, final String[] array) {
        ...
        player.sendMessage(" ");
        player.sendMessage(" ");
        player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l[&a&lThePit&f&l] &7Pit Random Gold wand was set on &aSlot 1&7."));
        API.getVersionSupport().sendTitle(player, VersionSupport.TitleType.TITLE, "", 5, 10, 5);
        API.getVersionSupport().sendTitle(player, VersionSupport.TitleType.SUBTITLE, "&7Pit Random Gold wand was set on &aSlot 1&7.", 10, 20, 10);
    ...
    private void onPlayerInteract(final PlayerInteractEvent playerInteractEvent) {
        ...
        switch (playerInteractEvent.getAction()) {
            case RIGHT_CLICK_BLOCK: {
                ...
                player.sendMessage(" ");
                player.sendMessage(" ");
                if (Main.getRandomGold().isSet(location)) {
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l[&a&lThePit&f&l] &7Pit &fGold Location &7is &calready set &7to your clicked location."));
                    API.getVersionSupport().sendTitle(player, VersionSupport.TitleType.TITLE, "", 5, 10, 5);
                    API.getVersionSupport().sendTitle(player, VersionSupport.TitleType.SUBTITLE, "&7Pit &fGold Location &7is &calready set &7to your clicked location.", 10, 20, 10);
                    Sounds.VILLAGER_NO.getSound().play(player, 3.0f, 1.0f);
                    return;
                }
                Main.getRandomGold().add(playerInteractEvent.getClickedBlock().getLocation());
                Bukkit.getPluginManager().callEvent(new SetupSetEvent(SetupUtils.SetupType.RANDOM_GOLD, player));
                player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&f&l[&a&lThePit&f&l] &7Pit &fGold Location &7was &aadded &7to your clicked location."));
                API.getVersionSupport().sendTitle(player, VersionSupport.TitleType.TITLE, "", 5, 10, 5);
                API.getVersionSupport().sendTitle(player, VersionSupport.TitleType.SUBTITLE, "&7Pit &fGold Location &7was &aadded &7to your clicked location.", 10, 20, 10);
